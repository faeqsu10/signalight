...
class TestIntradayRestartRecovery:
    """장중 재시작 후 후보 캐시가 비면 1회 재스캔으로 복구되는지 확인."""

    def test_kr_intraday_monitor_rescans_once_when_cache_missing(self):
        """KR 파이프라인은 캐시가 비고 오늘 스캔 기록이 없으면 1회 재스캔한다."""
        from autonomous.pipeline import AutonomousPipeline
        ...
        pipeline.run_morning_scan = MagicMock(
            side_effect=lambda: pipeline._daily_candidates.append({"ticker": "005930"})
        )
        ...
        result = pipeline.run_intraday_monitor()

        pipeline.run_morning_scan.assert_called_once_with()
        assert result["warning_count"] == 0
        assert result["error_count"] == 0

    def test_kr_intraday_monitor_skips_rescan_after_today_scan(self):
        """KR 파이프라인은 오늘 이미 스캔했다면 빈 후보여도 재스캔하지 않는다."""
        from autonomous.pipeline import AutonomousPipeline
        ...
        pipeline.run_morning_scan.assert_not_called()

    def test_us_intraday_monitor_rescans_once_when_cache_missing(self):
        """US 파이프라인도 장중 재시작 후 1회 재스캔으로 후보 캐시를 복구한다."""
        ...
        pipeline._record_ops_event = MagicMock()
        pipeline.run_morning_scan = MagicMock(
            side_effect=lambda: pipeline._daily_candidates.append({"ticker": "AAPL"})
        )
        pipeline.tracker = MagicMock()
        pipeline.tracker.get_all_open.return_value = []
        pipeline.analyzer = MagicMock()
