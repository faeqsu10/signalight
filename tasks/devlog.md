...
## Phase 14 — 룰 기반 매매 추천 시스템 [완료]

| # | 항목 | 상태 | 비고 |
|---|------|------|------|
| 117 | config.py 매매 룰 설정값 외부화 | ✅ | 레짐별 진입/손절/목표, 포트폴리오 제한 |
| 118 | trading/rules.py 룰 엔진 | ✅ | should_buy + should_sell + 게이트/필터 |
| 119 | trading/position_tracker.py 가상 포지션 추적 | ✅ | SQLite, 분할매수, 트레일링스탑 |
| 120 | formatter.py 추천 액션 섹션 | ✅ | 시그널 알림 + 브리핑에 💡 추천 표시 |
| 121 | main.py 통합 | ✅ | 룰 판단 + 가상 포지션 자동 관리 |
